...
** SuperScalar Architectures
   ...
   The most ideal architecture to schedule for would be a RISC architecture with
   $n$ evenly replicated functional units (as seen in
   [[fig:SuperScalarArchitecture]]). One way to view optimizing a schedule over such
   an architecture would be to maximize the amount of live functional units per
   cycle (i.e maximum throughput). This would generally be accomplished by
   interleaving different types of instruction, however if you stretch data
   dependent instructions too far apart doing this you risk running out of
   available registers (you increase register pressure). This is further
   complicated on CISC architectures (unsurprisingly) because of heterogeneous
   instruction lengths. 
