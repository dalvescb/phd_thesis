#+TITLE: Analysis of Compiler Heuristics via Stochastic Scheduling and Hyper-Heuristics 
...
** Out-of-Order Execution
   ...
   *Out-Of-Order Exeuction*
    ...
    4. Dispatched once operands are available (even if before an older
       instruction)
    5. Execute (on appropriate functional unit)
    6. Results are retired to another temporary queue
    7. Only after all older instructions have their results written back, then
...
/distance/) and assigning 0 to all $f_i$. A *Key Idea* to this work: encode
...
** Stochastic Scaling
   ...
   - *IDEA* scale the *IO penalty* stochastic-ally by a multiple of the II
   \begin{align}
