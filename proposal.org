...
** Out-of-Order Execution
   ...
   two paradigms: *in-order* or *out-of-order* (also known as dynamic)
   execution. Execution of an instruction cycle in each paradigm comprises of
   different steps:
   
   *In-Order Execution*
    1. instruction fetch
    2. stall until all operands are available and then dispatched
    3. Execute (on appropriate functional unit)
    4. Write back to register file
   
   *Out-Of-Order Exeuction*
    1. Instruction fetch
    2. Dispatch to a temporary queue known as *Reservation Station*
    3. Waits in the reservation station until operands are available
