...
In compiler optimization, \textbf{Instruction Scheduling} seeks to optimize the order of
...
each type of functional unit. Figure \ref{fig:PipelinedArchitecture} show's an
...
\label{sec:org70f2515}
Both of the previous sections figures \ref{fig:PipelinedArchitecture} and
...
\item \textbf{Control Hazards} also known as Branch Hazards, occurs when a bad branch
...
scheduling is attempted. The \textbf{Minimum Initiation Interval} or \textbf{MII} is a lower
bound on the possible value of any II for which a modulo schedule exists. The
MII is constrained by both resource constraints (\textbf{resMII}) and recurrence
constraints (\textbf{recMII}). The resource constraint simply holds that resources
...
execution. Execution of an instruction cycle in each paradigm comprises of
different steps:
...
\item Dispatched once operands are available (even if before an older
instruction)
\item Execute (on appropriate functional unit)
\item Results are retired to another temporary queue
\item Only after all older instructions have their results written back, then
...
Tomasulo's Algorithm innovated upon score-boarding allowing improved parallel
...
depends on the heuristic, but is generally polynomial
...
\section{Linear/Constraint Programming}
\label{sec:org105d44b}
...
\parencite{kastner1999code}). Solving an ILP problem is an NP-Complete, so
...
Another approach which has been far less investigated would be to "relax" the
problem to a continuous Linear Program with constraints. Notable efforts into
this approach are the works of McInnes/Beek in
\parencite{malik2008optimal},\parencite{van2001fast}. The constraint LP
problem in \parencite{malik2008optimal} found provably optimal schedules for
basic blocks , with the following types of constraints:
\begin{itemize}
...
notable as it illustrates how a relaxed LP problem can be used to model the
space of schedules. Even with the limitations on OoO architectures cutting of
the optimal schedule, it's reasonable to assume the LP would find
near-optimal schedules. And by converting hard constraints to soft
constraints (penalties) its simple to assert this space would contain the
optimal solution.

\section{Stochastic Search}
\label{sec:orgc820b56}
There is a notable work to explore the space of schedules through a
stochastic algorithm via Schkufza, Sharma, Aiken at Stanford
...
Although the cost function doesn't model the space of valid schedules,
using it to generate schedules along with a may still prove to be the most
practical way to explore a variety of schedules. 

\section{Meta-Optimization / Hyper-Heuristics}
\label{sec:orgc850723}
...
\label{sec:org4ae54c1}
In this section I will introduce a Stochastic Scheduling Algorithm that
utilizes a continuous optimization model with stochastic-ally generated
...
the probability the instruction \(i\) should spill. The constant variable \(II\) is
...
used in the Hard Constraints equations above denotes instruction \(j\)
depends on instruction \(i\). The first hard constraint enforces any 
...
fall within. And the third constraint enforces an instruction must complete
within the same interval it was dispatched. 
...
\(\epsilon\) (a small constant chosen for the purpose of creating any arbitrary
\emph{distance}) and assigning 0 to all \(f_i\). A \textbf{Key Idea} to this work: encode
...
penalizing the dispatch time of instructions based o the quantity and
...
alteration of the famous sigmoid function often used in neural networks)
...
\label{sec:org69f2097}
The scaling \(\frac{1}{N_j}\) may be a good guess, but not necessarily effective in practice
\begin{itemize}
\item \textbf{IDEA} scale the \textbf{IO penalty} stochastic-ally by a multiple of the II
\end{itemize}
\begin{align}
...
stochastic-ally, in basically the same manner as the IO penalty. The nature
...
overlapping heuristics. By scaling these heuristics stochastic-ally, we
already have a method to analysis heuristic selection through analyzing which
...
results order to judge the influence of penalties on a given architecture.
